...
local script_url = 'https://github.com/TankerVScripte/DiChat/blob/main/DiChat.luac' -- Путь скрипту.
...
-- функция проверки работы скрипта и обновления
function main()
	...
	-- статус валидности версии
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            updateini = inicfg.load(nil, update_path)
            if tonumber(updateini.info.vers) > script_version then
                sampAddChatMessage("Есть обновление! Версия {FA8072}" .. updateini.info.script_version, -1)
                update_state = true
            end
...
-- функция работы с обновлением и основынм функционалом скрипта
