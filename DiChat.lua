...
{DFCFCF}[Подсказка] {DC4747}Советуем пройти начальные квесты у Фермера, так вы сможете заработать первые деньги и познакомиться с игрой.
[Гонка вооружений]
[Безумная охота]
[Arizona Rent]
...
function se.onServerMessage(color, text)
	...
	local messages = {
		...
		["Вы взяли инкассаторский фургон. Выберите территорию с которой хотите забрать деньги и отправляйтесь на место. Будьте осторожны, вас могут ограбить!"] = "{73B461}[Информация] {ffffff}Вы взяли семейную финковозку.",
		["Вы вернули инкассаторский фургон в автопарк семьи."] = "{73B461}[Информация] {ffffff}Вы вернули семейную финковозку в автопарк семьи."
    }
...
--[[
   ...
   -- поздравление с апом уровня в фаме
	if text:find("%[Новости Семьи]{FFFFFF} Член семьи: .+_.+%[.+] достиг .+ уровня%. В семью начислен опыт%.") then
		local famNick = text:match("Член семьи: (.+)%[")
		local myNick = sampGetPlayerNickname(sampGetLocalPlayerId())

		if famNick ~= myNick then
			lua_thread.create(function()
				wait(1000)
				sampSendChat("/fam С днём рождения!")
			end)
